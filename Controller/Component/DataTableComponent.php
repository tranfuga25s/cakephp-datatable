...
class DataTableComponent extends PaginatorComponent {
    ...
    public function process($object = null, $scope = array()) {
        ...
        $query = array_diff_key($settings, array(
    ...
    protected function _parseSettings($object = null) {
        ...
        // Checks if the configuration has been already parsed
    ...
    /**
     * Changes the pagination to include search filters
     * This should be compatible with normal filtering setted in the component
     * and with the Cake Search Plugin
     * @param type $settings settings array
     */
    private function _conditionals( $settings ) {
        // Search if the request has some filtering setted
        if( array_key_exists( 'conditions', $this->settings ) ) {
            // Check for another fields to add to the column names
            /// @TODO: Check for another fields
            
            // Search if is needed another field to search with
